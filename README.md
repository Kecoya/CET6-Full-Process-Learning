...
六大题型 AI 训练应用（出题 · 做题 · 评分 · 倒计时）+ 自由学习离线工具
...
- **做题训练（6 个 Flask 应用）**：听力、选词填空、长篇阅读、仔细阅读、翻译、写作——每个题型一个独立应用，由 DeepSeek 现场生成与真题高度一致的题目，并提供**出题 → 做题 → 评分 → 倒计时**完整闭环。
...
> 六个应用结构同构（`app.py` + `templates/` + `static/` + `prompts/` + `.env.example` + `requirements.txt`），各自独立运行。学习时一次启动一个即可。
