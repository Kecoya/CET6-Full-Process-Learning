    ...
    async function generateExercise() {
        ...
        try {
            ...
            toast(savedName ? ('已生成并存档：my/' + savedName) : '生成完成，开始翻译', 'ok');
    ...
    function loadExercise(d) {
        d.topic = d.topic || 'CET-6 翻译';
        state.exercise = d;
        state.answer = ''; state.scored = false;
        document.getElementById('ansBadge').textContent = '🌐 ' + (d.topic || '');
        document.getElementById('sourceBox').innerHTML = '<div class="source-box">' + escapeHtml(d.source) + '</div>';
        document.getElementById('answerInput').value = '';
        updateWordCount();
        document.getElementById('hintsBox').classList.add('hidden');
        document.getElementById('hintBtn').textContent = '💡 显示术语提示';
    ...
    function updateWordCount() {
        ...
        el.textContent = n + ' 词';
        el.className = 'word-counter ' + (n < 40 ? 'short' : (n <= 120 ? 'ok' : 'long'));
    }
    ...
    function toggleHints() {
        ...
        if (box.classList.contains('hidden')) {
            const terms = (ex.key_terms || []).map(t => `<li><b>${escapeHtml(t.cn)}</b> → ${escapeHtml(t.en)}${t.note ? '（' + escapeHtml(t.note) + '）' : ''}</li>`).join('');
            box.innerHTML = `<h5>📌 关键术语参考（可作提示）</h5><ul>${terms || '<li>无</li>'}</ul>`;
            box.classList.remove('hidden');
            btn.textContent = '🙈 隐藏术语提示';
        } else {
            box.classList.add('hidden');
            btn.textContent = '💡 显示术语提示';
        }
    }
    ...
    async function submitScore() {
        ...
        if (!answer) { if (!confirm('你还没有写译文，确定要提交评分吗？')) return; }
        stopTimer();
        state.answer = answer;
        showLoading('AI 正在评估你的译文…');
        try {
    ...
    function renderScore(r) {
        ...
        const missed = (Array.isArray(r.missed_terms) ? r.missed_terms : []).map(x => `<li>${escapeHtml(String(x))}</li>`).join('');
        ...
        document.getElementById('scoreContent').innerHTML = `
            <div class="score-wrap">
                <div class="score-ring" id="scoreRing" style="--p:0%"><div class="inner"><div class="num">${score}</div><div class="lbl">翻译得分 / 100</div></div></div>
                <div class="level-badge level-${escapeHtml(level)}">${escapeHtml(level)}</div>
                <div class="eval-grid">
                    <div class="eval-card"><h5>🎯 准确性</h5><p>${escapeHtml(r.accuracy || '—')}</p></div>
                    <div class="eval-card"><h5>🌊 流畅性</h5><p>${escapeHtml(r.fluency || '—')}</p></div>
                    <div class="eval-card"><h5>📕 术语运用</h5><p>${escapeHtml(r.term_use || '—')}</p></div>
                    ...
                    ${missed ? `<div class="eval-card eval-full"><h5>⚠️ 漏译/错译的关键术语</h5><ul class="missed-list">${missed}</ul></div>` : ''}
    ...
    function renderReference() {
        const ex = state.exercise;
        const terms = (ex.key_terms || []).map(t => `<li><b>${escapeHtml(t.cn)}</b> → ${escapeHtml(t.en)}${t.note ? '（' + escapeHtml(t.note) + '）' : ''}</li>`).join('');
        const notes = (ex.sentence_notes || []).map(s => `<li><b>${escapeHtml(s.source)}</b><br>${escapeHtml(s.analysis)}</li>`).join('');
        document.getElementById('referenceContent').innerHTML = `
            <div class="reference-box"><h4>📝 参考译文</h4>${escapeHtml(ex.reference || '')}</div>
            ${terms ? `<div class="key-terms-box"><h5>🔑 关键术语译法</h5><ul>${terms}</ul></div>` : ''}
            ${notes ? `<div class="cloze-notes"><h5>🧩 难句翻译技巧</h5><ul>${notes}</ul></div>` : ''}`;
    }
    ...
    function restart() {
        if (!confirm('开始新的一段？当前内容将被替换。')) return;
        stopTimer();
